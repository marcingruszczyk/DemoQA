...
export class ButtonsBoxPage {
  ...
  doubleClickOnButton = this.page.getByRole('button', { name: 'Double Click Me' });
  rightClickOnButton = this.page.getByRole('button', { name: 'Right Click Me' });
  leftClickOnButton = this.page.getByRole('button', { name: 'Click Me', exact: true });
  ...
  async assertButtonsMessages(doubleCLick: string, rightClick: string, leftClick: string): Promise<void> {
